      ...
      <div class="col-12">
        <QuizBackground class="quiz-add__quiz">
          <div class="quiz-add__container">
            ...
            <h1 class="quiz-add__title">Éditer son quiz</h1>
...
export default Vue.extend({
  ...
  async mounted() {
    try {
      let quiz = await this.$axios.$get(`/api/quiz/${this.code}`);
      quiz = quiz[0];
      this.name = quiz.name;
      let questions = await this.$axios.$get(`/api/question/${this.code}`);
      questions.forEach(question => {
        question.answers.forEach(answer => {
          answer['value'] = answer['title'];
          answer['isTrue'] = answer['isTrue'] == '0' ? false : true;
        });
      });
      this.questions = questions;
      this.progressMax = questions.length - 1;
    } catch (err) {
      console.log('oups');
    }
  },
  methods: {
    ...
    async save() {
      ...
      try {
        await this.$axios.$put(`/api/quiz/update`, {'quizcode' : this.code, 'name': this.name, 'questions': this.questions });
